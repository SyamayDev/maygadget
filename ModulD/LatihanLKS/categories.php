...
// Ambil produk dari database berdasarkan kategori yang dipilih
...
<head>
    ...
    <style>
        /* Tambahan styling untuk filter kategori */
        .category-filters {
            display: flex;
    ...
    <div class="container">
        <nav class="navbar">
            ...
            <div class="nav-links">
                ...
                <a href="categories.php" id="active">Categories</a>
...
<!-- Product Section dengan Filter Kategori -->
<section id="product1" class="section-p1">
    <div class="container">
        <h2>Produk Kategori <?php echo $selected_category === 'all' ? 'Semua' : htmlspecialchars($selected_category); ?></h2>
        <p>Pilihan Terbaik Untuk Anda</p>
        
        <!-- Filter Kategori -->
        <div class="category-filters">
            <a href="categories.php?category=all<?php echo isset($_GET['search']) ? '&search=' . urlencode($_GET['search']) : ''; ?>" class="category-btn <?php echo $selected_category === 'all' ? 'active' : ''; ?>">Semua Produk</a>
            <?php 
            foreach ($categories as $category) {
                $is_active = $selected_category === $category ? 'active' : '';
                echo '<a href="categories.php?category='.urlencode($category).(isset($_GET['search']) ? '&search=' . urlencode($_GET['search']) : '').'" class="category-btn '.$is_active.'">'.$category.'</a>';
            }
            ?>
        </div>
        ...
        <?php if (!empty($search_results)): ?>
            <div class="pro-container">
                <?php foreach ($search_results as $product): ?>
                    <div class="pro">
                        <div class="favorite-icon" data-product-id="<?php echo $product['id']; ?>" onclick="toggleFavorite(<?php echo $product['id']; ?>, this)">
        ...
        <?php else: ?>
            <div class="pro-container">
                <?php if (count($products) > 0): ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <p style="grid-column: 1 / -1; text-align: center;">Tidak ada produk ditemukan untuk kategori ini</p>
                <?php endif; ?>
            </div>
        <?php endif; ?>
    </div>
